...
<body>
    <h1>
        <a href="https://github.com/jay/parascan" target="_blank" rel="noopener"
           style="color:inherit;text-decoration:none;display:inline-flex;align-items:center">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#4ade80" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle;margin-right:0.25rem">
    ...
    <p class="meta" style="margin-top:2rem">
        Generated by <a href="https://github.com/jay/parascan" target="_blank" rel="noopener"
        style="color:#60a5fa;text-decoration:none">parascan</a> v0.1.0
        at {datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S UTC')}
    </p>
</body>
</html>"""
...
def _build_scan_stats_html(stats: dict, scan: Any, soft404_count: int = 0) -> str:
    ...
    module_chips = " ".join(
        f'<span style="display:inline-block;background:#1e293b;padding:0.15rem 0.4rem;'
        f'border-radius:9999px;font-size:0.75rem;color:#94a3b8;margin:0.1rem;">'
        f'{_escape(m)}: {c}</span>'
        for m, c in sorted(by_module.items())
    )

    status_items = ""
    status_colors = {"2xx": "#4ade80", "3xx": "#60a5fa", "4xx": "#fde68a", "5xx": "#f87171", "error": "#6b7280"}
    ...
    <div style="background:#1e293b;border:1px solid #334155;border-radius:8px;padding:1rem;margin-bottom:1rem;">
        <p><strong>{total}</strong> HTTP request(s) made during scan
        {f'&mdash; duration: {duration_str}' if duration_str else ''}</p>
        <p style="margin-top:0.5rem">Responses: {status_items}</p>
        <p style="margin-top:0.5rem">By module: {module_chips}</p>
    </div>
    """


def _build_diagnostics_html(events: list) -> str:
    """build scan diagnostics appendix for the HTML report.

    Only includes warning/error events — informational events are omitted
    from reports to keep them focused for security engineers.
    """
    notable = [e for e in events if e.level in ("warning", "error")]
    if not notable:
        return ""

    level_colors = {"error": "#f87171", "warning": "#fde68a"}

    rows = ""
    for e in notable:
        color = level_colors.get(e.level, "#94a3b8")
        time_str = e.timestamp.strftime("%H:%M:%S") if e.timestamp else ""
        detail_html = (
            f'<br><span style="color:#64748b;font-size:0.8rem">{_escape(e.detail)}</span>'
            if e.detail else ""
        )
        rows += (
            f'<tr><td>{time_str}</td>'
            f'<td style="color:{color};font-weight:600">{_escape(e.level)}</td>'
            f'<td>{_escape(e.category)}</td>'
            f'<td>{_escape(e.message)}{detail_html}</td></tr>'
        )

    return f"""
    <h2>Scan Diagnostics</h2>
    <p style="color:#64748b;font-size:0.85rem;margin-bottom:0.5rem;">
        Warnings and errors encountered during the scan ({len(notable)} event(s)).
    </p>
    <table class="criteria-table">
